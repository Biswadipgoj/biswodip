...
const buildOpening: SceneBuilder = (tl, root, desktop) => {
  ...
  tl.to('.hero-intro', { y: -70, autoAlpha: 0, duration: 0.12, ease: 'power2.inOut' }, 0.08)
    ...
    .fromTo('.hero-editor', 
      { yPercent: desktop ? 50 : 40, scale: desktop ? 0.88 : 0.94, rotateX: desktop ? 8 : 0 }, 
      { yPercent: desktop ? -4 : -2, scale: 1.04, rotateX: 0, duration: 0.22, ease: 'power2.out' }, 
      0.10
    )
  ...
  tl.to('.hero-pipeline', { autoAlpha: 1, duration: 0.04 }, 0.49)
    ...
    .to('.hero-editor', { scale: 0.92, rotateX: desktop ? 12 : 0, y: -28, autoAlpha: 0, duration: 0.10, ease: 'power2.in' }, 0.68)
    ...
    .fromTo('.architecture-sheet', 
      { y: 60, scale: 0.92, rotateX: desktop ? 18 : 0, autoAlpha: 0 }, 
          ...
          <div className="architecture-sheets">
            {stack.map((layer, index) => (
              <div className={`architecture-sheet sheet-${index}`} key={layer.id}>
                ...
                <span className="attached-tech">{layer.tools.slice(0, 2).join(' / ')}</span>
              </div>
            ))}
          </div>
